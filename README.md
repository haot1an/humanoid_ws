...
量化发现，当前策略的左右脚支撑占比是 **0.76–0.86 / 0.17–0.27**，右脚摆动时抬到 36–48 cm，是一种"单腿支撑、另一条腿收起"的跛行步态。Isaac 和 MuJoCo 中一致，说明是训练本身学出来的。

候选原因：默认的 `feet_air_time` 奖励上限（0.6 s）大于对称步态每次单支撑的时长（约 0.35 s），不对称步态反而拿分更多。正在用单变量实验验证（上限改为 0.4 s，另设对照组）。

![左右脚高度与着地时序](media/gait_asymmetry_foot_trace.png)
...
rl/
  ...
  h1_locomanip/tasks/        Isaac Lab 环境：上肢 QP action term、MDP 项（课程 / 奖励 / 终止）、v2–v4 配置
  scripts/                   sim2sim 部署评估、契约导出、冒烟测试、热启动
  tests/                     torch vs C++ 一致性、课程离线测试
...
isaaclab.sh train --rl_library rsl_rl --task H1-LocoManip-CarryBoxRes-v4 --seed 42 \
    --external_callback h1_locomanip.tasks.register env.curriculum.payload.params.enabled=true

# MuJoCo 部署评估（策略权重不在仓库中，需要自行训练导出）
python rl/scripts/sim2sim_carry.py --policy <exported/policy.pt> --contract rl/contract/carry_v4.json --out eval
```

硬件：RTX 4080 SUPER，4096 个并行环境。带上肢 QP 与真实箱子时吞吐 23–28k steps/s，2000 轮约 2 h。
...
- 步态不对称（3.5 节）正在修正；后续计划用周期步态奖励，或者用真人动捕数据做风格先验（AMP）。
