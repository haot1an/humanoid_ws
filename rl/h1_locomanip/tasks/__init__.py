...
GAIT_TASK_ID = "H1-LocoManip-CarryBoxGait-v5"  # DEC-009：周期步态 + 左右镜像增强
...
def register():
    ...
    if GAIT_TASK_ID not in gym.registry:
        gym.register(
            id=GAIT_TASK_ID,
            entry_point="isaaclab.envs:ManagerBasedRLEnv",
            disable_env_checker=True,
            kwargs={
                "env_cfg_entry_point": f"{__name__}.carry_env_cfg:H1CarryBoxGaitEnvCfg",
                "rsl_rl_cfg_entry_point": f"{__name__}.carry_env_cfg:H1CarryBoxGaitPPORunnerCfg",
            },
        )
